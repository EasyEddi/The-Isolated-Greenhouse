# The Isolated Greenhouse

A peaceful first-person plant growing simulator set inside a large isolated greenhouse hall.

The player lives and works alone in a warm, quiet, slightly overgrown indoor greenhouse. There are no NPCs, no horror elements, and no stressful survival mechanics. The core loop is about growing mother plants, cutting offshoots, fulfilling one plant order at a time, and slowly building a bigger plant collection.

## MVP Goal

For the first version, the game should run in the browser and show the main loop in a simple but playable way:

1. View the greenhouse.
2. See the current customer order.
3. Grow or care for a plant.
4. Cut and package an offshoot.
5. Complete the order.
6. Unlock the next order.

The MVP should stay small. It does not need a huge 3D world, many plant species, login, or a database at the beginning.

## Team

- Eddi: greenhouse scene, player interaction, plant care flow
- Tarek: order/shop UI, progression, task tracking

## Documents

- [Unreal Startplan](STARTPLAN-UNREAL.md)
- [Product Brief](PRODUCT-BRIEF.md)
- [Team Workflow](TEAM-WORKFLOW.md)
- [Original Assignment](AUFGABE-WOCHE-2.md)
